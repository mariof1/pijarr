...
setup_nzbget() {
    ...
    # --- user & group setup --- #
    task_start "Adding ${app_user} service user…"
    if id "${app_user}" >/dev/null 2>&1; then task_pass "User account exists."; else useradd -s /usr/sbin/nologin -d "${app_lib_path}" -r -m -U "${app_user}"; check_result; fi
    task_start "Ensuring ${app_group} group…"; getent group "${app_group}" >/dev/null 2>&1 || groupadd "${app_group}"; check_result
    task_start "Adding ${app_user} to ${app_group}…"; id "${app_user}" | grep -qE "\b${app_group}\b" || usermod -a -G "${app_group}" "${app_user}"; check_result
    actual_user="${SUDO_USER:-$USER}"
    task_start "Adding ${actual_user} to ${app_group}…"; id "${actual_user}" | grep -qE "\b${app_group}\b" || usermod -a -G "${app_group}" "${actual_user}"; check_result

    # --- package install (with safety reinstall) --- #
    task_info "Installing ${app_name} package…"; pkg_install nzbget
    if ! command -v nzbget >/dev/null 2>&1; then
        task_warn "nzbget binary missing – forcing reinstall…"; apt-get -y --reinstall install nzbget;
    fi
...
remove_nzbget() {
    app_name="nzbget"
    app_lib_path="/var/lib/${app_name}"
    task_warn "You are about to delete all settings and files for ${app_name}…"; check_continue

    systemctl stop "${app_name}" 2>/dev/null
    rm -f /etc/systemd/system/"${app_name}".service
    systemctl daemon-reload

    task_info "Purging ${app_name} apt package…"; apt-get -y --purge remove nzbget
    deluser "${app_name}" 2>/dev/null
    rm -rf "${app_lib_path}"
    task_info "${app_name} deleted."
}
